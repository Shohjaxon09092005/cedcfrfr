...
def process_resource_video_only_sync(resource_id: str):
    """
    Synchronous VIDEO-ONLY pipeline (no test generation)
    File → Text → Script → Audio → Video (NO QUIZ)
    """
    ...
    try:
        ...
        if settings.KLING_API_KEY and audio_url_final:
            try:
                ...
                LessonResource.objects.filter(id=resource_id).update(url=video_url_final, video_url=video_url_final)
            except Exception as e:
        ...
        # ── DONE (NO QUIZ) ────────────────────────────────────────────────────
        update("ready", "✅ Video tayyor! Test alohida qilib yaratilsin.", video_url_final)
        return {"status": "success", "resource_id": resource_id, "video_url": video_url_final}

    except Exception as exc:
        error_msg = f"❌ Video pipeline xatosi: {str(exc)}"
        update("failed", error_msg)
        raise


@shared_task(bind=True, max_retries=2, default_retry_delay=30)
def process_resource_video_only(self, resource_id: str):
    """
    Async VIDEO-ONLY pipeline (no test generation)
    File → Text → Script → Audio → Video (NO QUIZ)
    """
    try:
        return process_resource_video_only_sync(resource_id)
    except Exception as exc:
        print(f"Video pipeline error: {exc}")
...
def process_resource_pipeline_sync(resource_id: str):
    ...
    try:
        ...
        try:
            ...
            # Mark that quiz was created
            LessonResource.objects.filter(id=resource_id).update(has_quiz=True)
        except Exception as e:
